...
def _ensure_dataset_downloaded(
    ...
    if not all_exist or force_download:
        logger.info(
            "Fashion-Mnist data not found locally. Downloading and preparing datasets..."
        )
        ...
        try:
            ...
            # Paths
            data_path = FASHION_TRAIN_DATA_PATH.parent.parent
            ...
            mnist_train, mnist_val = split_validation_dataset(
                ...
                save_method="parquet",
                set_name="fashion-mnist_train_e",
                val_name="fashion-mnist_validation",
                random_state=8,
                shuffle=True,
                stratify=True,
            )
            ...
            logger.info("✓ Fashion-Mnist data successfully prepared")
            return mnist_train, mnist_test, mnist_val

        except Exception as e:
            logger.error(f"Failed to download/prepare Fashion-Mnist data: {e}")
            raise LoadError(
                "Failed to download Fashion-Mnist dataset. "
                "Please check your internet connection and try again."
            ) from e

    else:
        # Load existing files
        logger.info("Loading Fashion-Mnist data from disk...")
        ...
        try:
            ...
            logger.info("✓ Fashion-Mnist data successfully loaded")
            return mnist_train, mnist_test, mnist_val
...
def load_fashion_mnist_data(
    ...
    force_download: bool = False,
) -> tuple[FashionData, FashionData, FashionData]:
    """
    Load Fashion-Mnist dataset with robust error handling and validation.
    ...
    Returns:
        tuple: (train, test, val) where each is a FashionData instance
    ...
    Examples:
        ...
        >>> train, test, val = mnist.load_fashion_mnist_data(
        ...     as_tensor=True,
        ...     tensor4d=True,
        ...     dtype=nova.float32,
        ... )
    ...
    try:
        ...
        train_path = Path(train_path or FASHION_TRAIN_DATA_PATH)
        test_path = Path(test_path or FASHION_TEST_DATA_PATH)
        val_path = Path(val_path or FASHION_VALIDATION_DATA_PATH)
...
def load_fashion_mnist_default() -> tuple[FashionData, FashionData, FashionData]:
    """
    Load Fashion-Mnist with recommended default settings.
    ...
    Example:
        >>> train, test, val = load_fashion_mnist_default()
        >>> train.x.shape
        (50000, 1, 28, 28)
    """
