 ...
 * device-to-device fast path for 1-D tensors. Launch geometry comes
 * from @ref resolve_launch_config() fed with the detected device
 * properties, so no per-kernel magic numbers are needed.
 *
 * @see launch_config.h  Shared grid/block resolution.
...
#include <ncore/native/kernels/launch_config.h>
...
void contiguousKernelOneDimTensor(const Tensor *src, Tensor *dst,
                                  novaStatus_t *st) {
 ...
 *
 * @details
 * Grid-stride kernel over raw byte buffers: each thread copies every
 * element whose index equals @c threadIdx.x + @c blockIdx.x *
 * @c blockDim.x modulo the total grid stride, stepping @p stride
 * source bytes per element. Takes plain data instead of @c Tensor so
 * no extended-precision type crosses into device code. The caller
 * validates @p cv beforehand on the host.
 *
 ...
 */
__global__ void contiguousKernelOneDimCV(size_t stride, size_t count,
                                         size_t item_size,
                                         const unsigned char *src_data,
                                         unsigned char *dst_data) {
  const size_t idx =
      (size_t)threadIdx.x + ((size_t)blockDim.x * (size_t)blockIdx.x);
  const size_t step = (size_t)blockDim.x * (size_t)gridDim.x;

  for (size_t item = idx; item < count; item += step) {
    const unsigned char *s = src_data + (item * stride);
 ...
 *
 * @details
 * Grid-stride kernel over the flat element index. Each thread
 * rebuilds its coordinates with
 ...
 *
 * @details
 * Already-contiguous sources become views; 1-D tensors take the
 * device-to-device fast path; anything else is collapsed and copied
 * by a grid-stride kernel sized with @ref resolve_launch_config()
 * from the detected device properties.
 *
...
novaStatus_t launchCudaContiguousKernel(const Tensor *src, Tensor *dst) {
  ...
  const LaunchConfig cfg = resolve_launch_config(
      src->size, src->item_size,
      static_cast<unsigned int>(props.maxThreadsPerBlock),
      static_cast<unsigned int>(props.multiProcessorCount));
  dim3 blocks(cfg.threads, 1, 1);
  dim3 grid(cfg.blocks, 1, 1);

  if (cv.ndims == 1) {
    if (cv.strides[0] != src->item_size) {
      contiguousKernelOneDimCV<<<grid, blocks>>>(cv.strides[0], src->size,
                                                 src->item_size, src->data.data,
                                                 dst->data.data);
    } else {
      st = cudaTransfer(src->storage->size_bytes,
                        DeviceMemcpyKind::deviceMemcpyDeviceToDevice,
                        src->data.v, dst->data.v);
      if (st.err != novaSuccess) {
        return st;
      }
    }
  } else {
    contiguousKernelNDim<<<grid, blocks>>>(cv, src->offset, src->size,
                                           src->item_size, src->data.data,
                                           dst->data.data);
  }
  st.err = novaSuccess;
