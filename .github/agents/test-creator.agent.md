...
argument-hint: "A file, function, or module to test — e.g. 'suggest tests for ncore/core/tensor.h' or 'add GTest coverage for the CPU dtype casting backend'"
...
The skills under `.agents/skills/` define NovaNN standards. Consult the relevant skill before working in the corresponding area:

| Area | Skill |
|------|-------|
| C++ / GoogleTest (all C++ test work) | [gtest-cpp23](../../.agents/skills/gtest-cpp23/SKILL.md) |
| C++23 language features | [cpp23-features](../../.agents/skills/cpp23-features/SKILL.md) |
| C23 language features | [c23-features](../../.agents/skills/c23-features/SKILL.md) |
| Python docstrings | [python-docstrings](../../.agents/skills/python-docstrings/SKILL.md) |
| CMakeLists.txt documentation | [cmake-rst-documentation](../../.agents/skills/cmake-rst-documentation/SKILL.md) |

### 1.2 Project instructions

`AGENTS.md` is loaded automatically into every agent session in this workspace. It is the authoritative architectural reference for the v5.0.0 design: tech stack, core subsystems, hardware backends, build system, and project status. Do not re-read it, and do not contradict it. `AGENTS.md` describes the target design, which differs from the current state of the code; verify specific APIs and signatures against the actual source code.
...
Use the `web` tool to verify framework APIs and best practices (GoogleTest, pytest, C++23, CMake) against official documentation, e.g. GoogleTest docs, pytest docs, cppreference. Do not write tests based on unverified assumptions.
...
Request permission before executing any command, stating exactly what will be run. Once approved:
...
- Do not execute any command without approval. The `execute` tool exists solely to enable verification; request permission and state the exact command first.
