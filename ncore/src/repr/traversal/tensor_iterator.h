 ...
 * @brief Public interface for the multidimensional strided tensor
 *        iterator.
 *
 * @details
 * Declares @ref TensorIterator and its associated control functions.
 * The iterator provides a high-level abstraction for traversing
 * tensors of any physical layout (contiguous or strided) in a
 ...
 *
 * @details
 * Tracks the current position within an n-dimensional tensor. The
 * iterator is considered "done" when all elements in the logical
 * shape have been visited.
 */
typedef struct {
...
/**
 * @brief Query whether the iterator has reached the end of the
 *        tensor.
 *
