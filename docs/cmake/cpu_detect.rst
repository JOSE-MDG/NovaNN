...
.. cmake:command:: check_simd
   ...
   #. Saves ``CMAKE_REQUIRED_FLAGS``.
   #. Appends ``FLAG``.
   #. Runs ``check_cxx_source_runs(SNIPPET VAR)``.
   #. Restores ``CMAKE_REQUIRED_FLAGS``.
   #. If ``VAR`` is true, appends ``FLAG`` to ``SIMD_FLAGS``.
...
Probes AVX-512 Foundation first, then six sub-extensions in dependency order:
BW, DQ, VL, VNNI, FP16, BF16.  All sub-extensions are set to 0 if AVX-512F
is unsupported.
