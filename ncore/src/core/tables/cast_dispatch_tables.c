 ...
 *
 * @details
 * Populates the NUM_DTYPES×NUM_DTYPES `cast_dispatch` matrix that maps every
 * pair of @ref DType_ values (source → target) to a type-specific cast kernel.
 * The table is populated once at load time via a
 ...
 *
 * The 90 non-null entries are organised into six groups:
 */
// clang-format off
/**
 ...
 * storage type (`int8` / `uint8`) with `Signed8` / `UnSigned8`
 * and are therefore covered by other cast kernels.
 *
 ...
 * @post All 90 non-trivial entries of `cast_dispatch` are set to
 *       valid function pointers.
 *
 * @see cast_dispatch   The table being populated.
 * @see castFn          Function pointer type.
 */
ATTR(constructor) static inline void init_cast_dispatch() {
