# How one spec maps to four harnesses

Start from `spec-template.yaml`. Don't copy the same block verbatim.

| Concept | Claude Code `.claude/agents/*.md` | OpenCode `.opencode/agents/*.md` | Codex `.codex/agents/*.toml` | GitHub `.github/agents/*.agent.md` |
| --- | --- | --- | --- | --- |
| Name | `name:` frontmatter | filename | `name =` | `name:` frontmatter (stem without `.agent`) |
| Trigger | `description:` | `description:` | `description =` | `description:` |
| Behavior | body | body | `developer_instructions` | body (prompt) |
| Scope | `tools:` | `permission:` + `mode:` | `sandbox_mode:` | `tools: [read, search, web]` |
...
| Intent | Claude Code | OpenCode | Codex | GitHub |
| --- | --- | --- | --- | --- |
| Read-only | `tools: Read, Grep, Glob` | `permission: {edit: deny}` | `sandbox_mode = "read-only"` | `tools: [read, search, web]` |
| Read-write | omit `tools` or allow `Read, Edit, Write, Bash` | `permission: {edit: allow}` | `sandbox_mode = "workspace-write"` | `tools: [read, search, edit, execute, web]` |
