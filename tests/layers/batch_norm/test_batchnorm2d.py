...
def test_batchnorm2d_forward_train_mode():
    ...
    x = RNG.randn(N, num_channels, H, W).astype(np.float32)
...
def test_batchnorm2d_forward_eval_mode():
    ...
    bn.train()
    x_train = RNG.randn(32, num_channels, 16, 16).astype(np.float32)
    ...
    # Different input
    x_eval = RNG.randn(8, num_channels, 16, 16).astype(np.float32)
    output = bn(x_eval)
...
def test_batchnorm2d_backward_gradient_check():
    ...
    x = RNG.randn(N, num_channels, H, W).astype(np.float32) * 0.1
...
def test_batchnorm2d_momentum_and_eps():
    ...
    x = RNG.randn(4, num_channels, 8, 8).astype(np.float32)
    bn.train()
    output = bn(x)
...
def test_batchnorm2d_different_spatial_sizes():
    ...
    for h, w in spatial_sizes:
        x = RNG.randn(2, num_channels, h, w).astype(np.float32)
        output = bn(x)
